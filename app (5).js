...
function renderPaymentsList(){
  ...
  $$('[data-del]', list).forEach(btn=>btn.addEventListener('click', ()=>{
    ...
    cacheRemove('payments', id);
    renderPaymentsList();
    deleteDoc(doc(db,'payments',id)).catch(()=> toast('No se pudo eliminar en el servidor, revisa tu conexión', true));
  }));
}
  ...
  $('#payment-form').addEventListener('submit', async (e)=>{
    ...
    try{
      ...
      if(isEdit){
        ...
        updateDoc(doc(db,'payments',existing.id), data).catch(()=> toast('No se pudo guardar en el servidor, revisa tu conexión', true));
      } else {
        ...
        renderPaymentsList();
        toast('Pago agregado');
        setDoc(ref, data).catch(()=> toast('No se pudo guardar en el servidor, revisa tu conexión', true));
      }
    }catch(ex){
...
function renderKanban(){
  ...
  $$('[data-quickadd]', container).forEach(input=>{
    ...
    const submit = ()=>{
      ...
      if(!title) return;
      input.value = '';
      const data = { listId, title, done:false, createdBy: CURRENT_EMAIL, createdAt: new Date().toISOString() };
      const ref = doc(collection(db,'tasks'));
      cacheSet('tasks', ref.id, data);
      renderKanban();
  ...
  $$('[data-dellist]', container).forEach(btn=>btn.addEventListener('click', ()=>{
    ...
    toRemove.forEach(t=>{ cacheRemove('tasks', t.id); deleteDoc(doc(db,'tasks',t.id)).catch(()=>{}); });
    cacheRemove('tasklists', listId);
    renderKanban();
    deleteDoc(doc(db,'tasklists',listId)).catch(()=> toast('No se pudo eliminar en el servidor, revisa tu conexión', true));
  }));
}
...
function openAddListModal(){
  ...
  $('#list-form').addEventListener('submit', async (e)=>{
    ...
    const ref = doc(collection(db,'tasklists'));
...
const renderVentas = guard(async function(main){
  ...
  main.innerHTML = `
    ...
    <div class="sale-grid" id="sales-list"></div>
...
function renderSalesList(){
  ...
  const filtered = saleFilter === 'Todas' ? all : all.filter(s=>s.category === saleFilter);
  const list = $('#sales-list');
  const pagerEl = $('#sales-pagination');
  if(filtered.length === 0){ list.innerHTML = '<div class="empty" style="grid-column:1/-1">Sin ventas en esta categoría.</div>'; pagerEl.innerHTML=''; return; }

  const totalPages = Math.max(1, Math.ceil(filtered.length / SALES_PER_PAGE));
  if(salePage >= totalPages) salePage = totalPages - 1;
  const docs = filtered.slice(salePage*SALES_PER_PAGE, salePage*SALES_PER_PAGE + SALES_PER_PAGE);
  ...
  list.innerHTML = docs.map(s=>{
    ...
    return `<div class="sale-card" data-sale="${s.id}">
      ...
      <div class="sale-card-foot">
        ...
        <button class="btn-danger" data-delsale="${s.id}">Eliminar</button>
      </div>
    </div>`;
  }).join('');

  pagerEl.innerHTML = totalPages > 1 ? `
    <button id="sales-prev" ${salePage===0?'disabled':''}>&larr;</button>
    <span>Página ${salePage+1} de ${totalPages}</span>
    <button id="sales-next" ${salePage>=totalPages-1?'disabled':''}>&rarr;</button>
  ` : '';
  if($('#sales-prev')) $('#sales-prev').addEventListener('click', ()=>{ salePage--; renderSalesList(); });
  if($('#sales-next')) $('#sales-next').addEventListener('click', ()=>{ salePage++; renderSalesList(); });

  $$('[data-instcheck]', list).forEach(chk=>chk.addEventListener('click', ()=>{
    ...
    renderSalesList();
    updateDoc(doc(db,'sales',saleId), {installmentsList: instList}).catch(()=> toast('No se pudo guardar en el servidor, revisa tu conexión', true));
  }));
  $$('[data-editsale]', list).forEach(btn=>btn.addEventListener('click', ()=>{
    openAddSaleModal((CACHE.sales||[]).find(x=>x.id===btn.dataset.editsale));
  }));
  $$('[data-delsale]', list).forEach(btn=>btn.addEventListener('click', ()=>{
    if(!confirm('¿Eliminar esta venta?')) return;
    const id = btn.dataset.delsale;
    cacheRemove('sales', id);
    renderSalesList();
    deleteDoc(doc(db,'sales',id)).catch(()=> toast('No se pudo eliminar en el servidor, revisa tu conexión', true));
  }));
}
  ...
  $('#sale-form').addEventListener('submit', async (e)=>{
    ...
    try{
      if(isEdit){
        ...
        updateDoc(doc(db,'sales',existing.id), data).catch(()=> toast('No se pudo guardar en el servidor, revisa tu conexión', true));
      } else {
        data.createdBy = CURRENT_EMAIL; data.createdAt = new Date().toISOString();
